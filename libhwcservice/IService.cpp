...
public:
    ...
    {
        // No longer done via binder.
        return new IDisplayControl(display);
    }

    virtual sp<IVideoControl> getVideoControl()
    {
        // No longer done via binder.
        return new BnVideoControl();
    }

    virtual sp<IMDSExtModeControl> getMDSExtModeControl()
    {
        // No longer done via binder.
        return new IMDSExtModeControl();
    }
...
IMPLEMENT_META_INTERFACE(Service, "intel.hwc.IService");


status_t BnService::onTransact(
        uint32_t code,
        const Parcel& data,
        Parcel* reply,
        uint32_t flags)
{
    switch(code) {
        ...
        {
            CHECK_INTERFACE(IService, data, reply);
#if ANDROID_VERSION >= 515
            sp<IBinder> b = IInterface::asBinder(this->getDiagnostic());
#else
            sp<IBinder> b = this->getDiagnostic()->asBinder();
#endif
...
#else
            sp<IBinder> b = this->getControls()->asBinder();
#endif
            reply->writeStrongBinder(b);
            return NO_ERROR;
        }
