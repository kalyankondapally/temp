...
#define INTEL_UFO_HWC_ASSERT_KEY_VALID(K)      \
  {                                            \
    ASSERT((K).length() > 0);                  \
    ASSERT(strchr((K).string(), '=') == NULL); \
  }
#define INTEL_UFO_HWC_ASSERT_VALUE_VALID(V) \
  do {                                      \
    ;                                       \
  } while (0)
#define INTEL_UFO_HWC_LOGE_OPEN \
  ETRACEIF(!mbOpen, "Persistent registry is not open")
...
void PersistentRegistry::open(void) {
  ...
  if (mbOpen) {
    ETRACE("Persistent registry is already open");
    return;
  }
...
void PersistentRegistry::close(void) {
  if (!mbOpen) {
    ETRACE("Persistent registry is not open");
    return;
  }
...
void PersistentRegistry::write(const HWCString& key, const HWCString& value) {
  INTEL_UFO_HWC_ASSERT_MUTEX_NOT_HELD(mLock);
  ...
  if (!mbOpen) {
    ITRACEIF(PERSISTENT_REGISTRY_DEBUG,
             "Persistent registry skipped write - closed");
    return;
  }
...
bool PersistentRegistry::read(const HWCString& key, HWCString& value) const {
  INTEL_UFO_HWC_ASSERT_MUTEX_NOT_HELD(mLock);
  INTEL_UFO_HWC_ASSERT_KEY_VALID(key);
  INTEL_UFO_HWC_ASSERT_VALUE_VALID(value);
  INTEL_UFO_HWC_LOGE_OPEN;
  ...
  if (!mbOpen) {
    ITRACEIF(PERSISTENT_REGISTRY_DEBUG,
             "Persistent registry skipped read - closed");
    return false;
  }
...
bool PersistentRegistry::read(const HWCString& key, char* pValue,
  ...
  if (numChars + 1 > maxChars) {
    ETRACE(
        "Persistent registry read key %s returned %s, exhausted maxChars %u "
        "[%u]",
...
HWCString PersistentRegistry::dump(void) const {
  INTEL_UFO_HWC_ASSERT_MUTEX_NOT_HELD(mLock);
  INTEL_UFO_HWC_LOGE_OPEN;
  ...
  if (!mbOpen) {
    ITRACEIF(PERSISTENT_REGISTRY_DEBUG,
             "Persistent registry skipped dump - closed");
    return HWCString("closed");
  }

  HWCString str;
  str = HWCString::format("saving:%d dirty:%d entries:%zu {", mbSaving,
                          (bool)mbDirty, mEntries.size());
  for (auto e : mEntries) {
...
void PersistentRegistry::saveToDisk( void )
{
...

    {
      ...
      if (!mbDirty) {
        ITRACEIF(PERSISTENT_REGISTRY_DEBUG,
                 "Persistent registry skipped save - no changes to save");
        return;
      }
      ...
      while (mbSaving) {
        if (mSignalSaveDone.waitRelative(mLock, timeoutNS) != NO_ERROR) {
          if (mbSaving) {
            ETRACE("Persistent registry wait for previous save timeout");
            return;
          }
        }
      }
      ...
      ITRACEIF(PERSISTENT_REGISTRY_DEBUG,
               "Persistent registry saving %zu entries [total %u chars]",
               mEntries.size(), numChars);
      ...
      if (pRegistry) {
        ...
        ITRACEIF(PERSISTENT_REGISTRY_DEBUG, "Persistent registry saving {");
        ITRACEIF(PERSISTENT_REGISTRY_DEBUG, " %s", pRegistry);
        ITRACEIF(PERSISTENT_REGISTRY_DEBUG, "}");
      } else {
        ETRACE("Persistent registry failed to alloc %u chars", numChars);
        return;
      }
    ...
    bool bOK = false;
    HWCString newFile = mCacheFilepath + ".new";
    ...
    if (fp) {
      ...
      if (w == 1) {
        ...
        if (rename(newFile.string(), mCacheFilepath.string()) == 0) {
          DTRACE("Persistent registry save %s x%u chars OK",
                 mCacheFilepath.string(), numChars);
          bOK = true;
        } else {
          ETRACE("Persistent registry rename %s -> %s failed %d/%s",
                 newFile.string(), mCacheFilepath.string(), errno,
                 strerror(errno));
        }
      } else {
        ETRACE("Persistent registry save %s x%u chars FAILED", newFile.string(),
               numChars);
      }
    } else {
      ETRACE("Persistent registry save %s x%u chars failed to open file",
             newFile.string(), numChars);
    }
...
void PersistentRegistry::loadFromDisk(void) {
  ...
  if (!fp) {
    ETRACE("Persistent registry load failed to open file %s",
           mCacheFilepath.string());
    return;
  }

  ITRACEIF(PERSISTENT_REGISTRY_DEBUG, "Persistent registry loading from %s",
           mCacheFilepath.string());
        ...
        if (pchKey && strlen(pchKey)) {
          *pchValue = '\0';
          ...
          if (strlen(pchValue) == 0) {
            ETRACE("Persistent registry [%s] malformed value at line %u", entry,
                   line);
          } else {
            DTRACEIF(PERSISTENT_REGISTRY_DEBUG, " Persistent registry %s=%s",
                     pchKey, pchValue);
            mEntries[HWCString(pchKey)] = pchValue;
          }
        } else {
          ETRACE("Persistent registry [%s] malformed key at line %u", entry,
                 line);
        }
      } else {
        ETRACE("Persistent registry [%s] malformed entry at line %u", entry,
               line);
      }
    }
  ...
  DTRACE("Persistent registry loaded %s created %zu entries from %u lines",
