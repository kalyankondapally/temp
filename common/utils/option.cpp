...
Option::Option(const char* propertyString, int32_t defaultValue,
               bool bForceGeometryChange)
    : mValue(0),
      mSourceType(SOURCE_TYPE_DEFAULT),
      mbInitialized(false),
...
Option::Option(const char* propertyString, const char* defaultValue,
               bool bForceGeometryChange)
    : mValue(0),
      mSourceType(SOURCE_TYPE_DEFAULT),
      mbInitialized(false),
...
void Option::initialize(const char* propertyString, int32_t defaultValue) {
  mbStringProperty = false;
  initializeInternal(propertyString, HWCString::format("%d", defaultValue));
}
...
void Option::initializeInternal(const char* propertyString,
                                const char* defaultValue) {
  mPropertyString = propertyString ?: "";
  mPropertyString.toLower();
  mPropertyStringAlternate.toLower();

  HWCString propertyName = HWCString(getPropertyRoot()) + mPropertyString;
  HWCString persistentRegistryName =
      HWCString(getPersistRoot()) + mPropertyString;

  HWCASSERT(propertyString && mPropertyString.length() > 0,
  ...
  HWCASSERT(mPropertyString.length() <= cNameLength,
  ...
  if (property_get(propertyName, value, NULL)) {
    // Property.
    mValueString = HWCString(value);
    mSourceType = SOURCE_TYPE_PROPERTY;
    // FIXME:
    // Log::add( "Option Forced %s (property:%s)", dump().string(),
    // propertyName.string( ) );
    // ITRACEIF(sbInternalBuild, "Option Forced %s (property:%s)",
    // dump().string(), propertyName.string( ) );
  } else if (property_get(mPropertyStringAlternate.string(), value, NULL)) {
    // Alternate property.
    mValueString = HWCString(value);
    mSourceType = SOURCE_TYPE_ALTERNATE_PROPERTY;
    // FIXME:
    // Log::add( "Option Forced  %s (alternate property:%s)", dump().string(),
    // mPropertyStringAlternate.string( ) );
    // ITRACEIF(sbInternalBuild, "Option Forced  %s (alternate property:%s)",
    // dump().string(), mPropertyStringAlternate.string( ) );
  }
  /* else if ( getPersistentRegistry().read( persistentRegistryName,
   mValueString ) )
   {
       // Persistent registry.
       mSourceType  = SOURCE_TYPE_PERSISTENT_REGISTRY;
       // FIXME:
      // Log::add( "Option Forced  %s (persistent registry:%s)",
   dump().string(), persistentRegistryName.string( ) );
       //ITRACEIF(sbInternalBuild, "Option Forced  %s (persistent
   registry:%s)", dump().string(), persistentRegistryName.string( ) );
   }*/
  else {
    // Build-time default.
    ...
    // ITRACEIF(sbInternalBuild, "Option Default %s (HWC default)",
    // dump().string());
  }
...
void Option::set(int32_t value) {
  HWCASSERT(!mbStringProperty, "set integer on a string property %s",
            mPropertyString.string());
  setInternal(HWCString::format("%d", value));
}

void Option::set(const char* value) {
  HWCASSERT(mbStringProperty, "set string on a integer property %s",
            mPropertyString.string());
  setInternal(value);
}

void Option::setInternal(const char* value) {
  HWCASSERT(mbInitialized, "Uninitialised access of %s",
            mPropertyString.string());
  // Log::alogi( "Changed option %s %s -> %s", mPropertyString.string( ),
  // mValueString.string(), value );
  mValueString = value;
  mbChanged = true;
  ...
  if (mbPersistent == true) {
    ...
    // Log::alogi(" Save persistent registry: %s = %s",
    // persistentRegistryName.string(), mValueString.string());
  }
...
HWCString Option::dump() const {
  HWCString output =
      HWCString::format("%*s : ", cNameLength, mPropertyString.string());
