    ...
    mbLogviewToLogcat(false)
{
    ...
    DTRACEIF(HWCLOG_DEBUG, "Log: Allocated HWC Log buffer %d bytes @%p",
             maxLogSize, mLogBuf);
...
char* BasicLog::reserve(uint32_t maxSize) {
  mLock.lock();
  DTRACEIF(HWCLOG_DEBUG, "Log: OpenNewLogEntry mBack=%p", mBack);
  char* possibleEntryEnd = mBack + maxSize;
  if (possibleEntryEnd > mLogBuf + mAllocatedSize) {
    if (mFront > mBack) {
      DTRACEIF(HWCLOG_DEBUG,
               "Log: Discarding remaining entries at %p and base entry at %p",
               mFront, mLogBuf);
  ...
  while ((mBack + maxSize > mFront) && (mBack < mFront)) {
    ...
    if (entrySize == 0) {
      ...
      DTRACEIF(HWCLOG_DEBUG, "Log: Discarding %d byte entry at %p", entrySize,
               mFront);
      mFront += entrySize;
    }

    *((uint16_t*)mFront) |= ENTRY_LOST_MASK;
    VTRACEIF(HWCLOG_DEBUG, "Log: Now front=%p", mFront);
  }
  DTRACEIF(HWCLOG_DEBUG, "Log: OpenNewLogEntry returning mBack=%p", mBack);
  return mBack + sizeof(uint16_t);
}

void BasicLog::log(char* endPtr) {
  DTRACEIF(HWCLOG_DEBUG, "Log: %zd byte entry written @ %p", endPtr - mBack,
           mBack);
  if ((mBack < mFront) && (endPtr > mFront)) {
    ETRACE("Log error : entry @ %p too big (%zd bytes) - resetting log", mBack,
           endPtr - mBack);
    if (endPtr > mLogBuf + mAllocatedSize) {
      ETRACE(
          "WARNING: Buffer at [%p - %p], entry ends @ %p, possible memory "
          "corruption",
          mLogBuf, mLogBuf + mAllocatedSize, endPtr);
    }
    mFront = mBack = mLogBuf;
  } else {
...
char* BasicLog::read(uint32_t& size, bool& lost) {
  ...
  if (mFront == mBack) {
    ...
    if (entryLost) {
      DTRACEIF(HWCLOG_DEBUG, "Log: Entry/ies lost");
    }

    if (entrySize < sizeof(uint16_t)) {
      ETRACE("Log error : Entry length %d at %p - resetting log", entrySize,
             mFront);
      mFront = mBack = mLogBuf;
      return 0;
    }

    DTRACEIF(HWCLOG_DEBUG, "Log: %d byte entry read at %p", entrySize, mFront);
    mFront += entrySize;
...
void Log::addInternal(uint32_t numDisplays,
  ...
  for (uint32_t d = 0; d < numDisplays; d++) {
    ...
    HWCString output = HWCString::format(
        "SF%u %s frame:%u Fd:%d outBuf:%p outFd:%d flags:%x", d, description,
        frameIndex, pDisp->retireFenceFd, pDisp->outbuf,
    ...
    for (uint32_t ly = 0; ly < pDisp->numHwLayers; ly++) {
      ...
      output.appendFormat(
          "\n  %d %s %s", ly,
          compositionTypeString(pDisp->hwLayers[ly].compositionType),
          layer.dump().string());
    }
...
void Log::addInternal(const Content::LayerStack& layers,
                      const char* description, va_list& args) {
  HWCString output = HWCString::format("%s", description) + layers.dumpHeader();
...
void Log::addInternal(const Content::LayerStack& layers, const Layer& target,
                      const char* description, va_list& args) {
  // Validation callback
...
void Log::addInternal(const Content::Display& display, const char* description,
                      va_list& args) {
  addInternal(display.getLayerStack(),
              HWCString::format("%s %s", description,
                                display.dumpHeader().string()).string(),
              args);
}

void Log::addInternal(const Content& content, const char* description,
                      va_list& args) {
  for (size_t d = 0; d < content.size(); d++) {
    Content::Display display = content.getDisplay(d);
    if (display.isEnabled()) {
      addInternal(display, HWCString::format("%s%zd", description, d).string(),
                  args);
    }
  }
}
...
status_t Log::readLogParcel(Parcel* reply) {
  ...
  if (spLog) {
    ...
    if (entry) {
      while (entry) {
        if (lost) {
          DTRACEIF(HWCLOG_DEBUG, "Log: Lost entries: status=eLogTruncated");
        ...
        DTRACEIF(HWCLOG_DEBUG,
                 "Writing log entry(%d) @ [%p:%d] to parcel %p total size "
                 "will be %d",
                 i, entry, size, reply, totalSize);
...
static uint32_t convertToHwc1Transform(ETransform transform) {
  ...
  switch (transform) {
    ...
    case ETransform::FLIP_H:
      t = HWC_TRANSFORM_FLIP_H;
      break;
    case ETransform::FLIP_V:
      t = HWC_TRANSFORM_FLIP_V;
      break;
    case ETransform::ROT_90:
      t = HWC_TRANSFORM_ROT_90;
      break;
    case ETransform::ROT_180:
      t = HWC_TRANSFORM_ROT_180;
      break;
    case ETransform::ROT_270:
      t = HWC_TRANSFORM_ROT_270;
      break;
    case ETransform::FLIP_H_90:
      t = HWC_TRANSFORM_FLIP_H | HWC_TRANSFORM_ROT_90;
      break;
    case ETransform::FLIP_V_90:
      t = HWC_TRANSFORM_FLIP_V | HWC_TRANSFORM_ROT_90;
      break;
  }
  return t;
}
...
static uint32_t convertToHwc1Blending(EBlendMode blend) {
  ...
  switch (blend) {
    ...
    case EBlendMode::PREMULT:
      b = HWC_BLENDING_PREMULT;
      break;
    case EBlendMode::COVERAGE:
      b = HWC_BLENDING_COVERAGE;
      break;
  }
  return b;
}

class InternalValLayer
    : public validation::AbstractCompositionChecker::ValLayer {
 public:
...
// Construct a temporary layer for passing to validation
// Caller must declare rects with enough space for
// layer.getVisibleRegions().size() entries
InternalValLayer::InternalValLayer(const Layer& layer) {
  ...
  planeAlpha =
      layer.getPlaneAlpha() * 255;  // Val expects alpha in the 0-255 range
}

void Log::validate(const Content::LayerStack& layers, const Layer& target,
                   const char* composer) {
