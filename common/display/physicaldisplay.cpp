...
PhysicalDisplay::PhysicalDisplay(GpuDevice& device) :
    mDevice( device ),
    //mPhysicalDisplayManager( mDevice.getPhysicalDisplayManager() ),
...
{
    DTRACEIF( PHYDISP_DEBUG, "Phy %p initializeOptions", this );
    ...
    mUserConfig.mMode.initialize( HWCString::format("%smode%d", prefix, optionIndex), "0x0@0-0" );
    mUserConfig.mMode.setForceGeometryChange( false );
    mUserConfig.mScalingMode.setPersistent(true);
    mUserConfig.mScalingMode.initialize( HWCString::format("%sscalemode%d", prefix, optionIndex), HWCS_SCALE_FIT );
    mUserConfig.mScalingMode.setForceGeometryChange( false );
    mUserConfig.mOverscan.setPersistent(true);
    mUserConfig.mOverscan.initialize( HWCString::format("%soverscan%d", prefix, optionIndex), "0x0" );
...
void PhysicalDisplay::setUserOverscan(int32_t xOverscan, int32_t yOverscan)
{
    ...
    mUserConfig.mOverscan.set( HWCString::format("%dx%d", xOverscan, yOverscan));
    //FIXME:
    /*
    // Implement Overscan via scaling filter.
    GlobalScalingFilter& scalingFilter = mDevice.getGlobalScalingFilter();
...
void PhysicalDisplay::setDisplayTimings( std::vector<Timing>& timings )
{
    ScopedSpinLock _l( mDisplayTimingsLock );

    std::vector<Timing>().swap(mDisplayTimings);
...
void PhysicalDisplay::copyDisplayTimings( std::vector<Timing>& timings ) const
{
...
int32_t PhysicalDisplay::getDefaultDisplayTiming( void ) const
{
    ...
    DTRACEIF( MODE_DEBUG, "getDefaultDisplayTiming" );
...
int32_t PhysicalDisplay::findDisplayTiming( const Timing& requested, uint32_t findFlags ) const
{
    ...
    DTRACEIF(MODE_DEBUG, "findDisplayTiming %s (Zero is wildcard): Flags %s|%s",
	...
	DTRACEIF(MODE_DEBUG, "findDisplayTiming Checking Timing %s", t.dump().string());
        ...
        {
	    DTRACEIF(MODE_DEBUG, "findDisplayTiming Timing %s Matches Geometry and Refresh", t.dump().string());
            matchedTimingIndex = i;
            break;
        }
        ...
        if (bMatchesGeometry && ( findFlags & FIND_MODE_FLAG_CLOSEST_REFRESH_MULTIPLE))
        {
            if ((matchedTimingIndex < 0) || (mDisplayTimings[matchedTimingIndex].getRefresh() > t.getRefresh()))
            {
                if ((t.getRefresh() % f ) == 0)
                {
		    DTRACEIF(MODE_DEBUG, "findDisplayTiming Timing %s Matches Geometry and Multiple of Refresh", t.dump().string());
                    matchedTimingIndex = i;
                }
            }
        }
    ...
    if ( matchedTimingIndex >= 0 )
    {
	DTRACEIF(MODE_DEBUG, "findDisplayTiming %s : Best match:%s", requested.dump().string(), mDisplayTimings[matchedTimingIndex].dump().string());
    }
    else
    {
	DTRACEIF(MODE_DEBUG, "findDisplayTiming %s : Did not find match", requested.dump().string());
    }
...
bool PhysicalDisplay::setSpecificDisplayTiming( uint32_t timingIndex, bool bSynchronize )
{
    ...
    {
	WTRACE("setSpecificDisplayTiming index %d is out-of-range", timingIndex );
        return false;
    }
    ...
    {
	DTRACEIF( MODE_DEBUG, "setSpecificDisplayTiming synchronize, index = %d, %s", timingIndex, t.dump().string());
	mDevice.synchronize( 0 );
	DTRACEIF( MODE_DEBUG, "setSpecificDisplayTiming synchronize complete, mode = %d, %s", timingIndex, t.dump().string());
    }
    else
    {
	mDevice.forceRedraw( );
...
bool PhysicalDisplay::setUserDisplayTiming( const Timing& timing, bool bSynchronize )
{
    ...
    mUserConfig.mMode.set( HWCString::format("%dx%d@%d-%x",timing.getWidth(), timing.getHeight(), timing.getRefresh(), timing.getRatio()) );
...
int PhysicalDisplay::onGetDisplayConfigs( uint32_t* paConfigHandles, uint32_t* pNumConfigs ) const
{
    ...
    DTRACEIF( PHYDISP_DEBUG || MODE_DEBUG,
    ...
    {
	ETRACE( "getDisplayConfigs : SF%x/P%u has Zero Configs", mSfIndex, getDisplayManagerIndex() );
...
{
    HWC_ASSERT_LOCK_NOT_HELD2(mTimingLock);
    if ( mNotifiedTimingIndex < mDisplayTimings.size() )
    {
        return mDisplayTimings[ mNotifiedTimingIndex ].getRefresh();
    }
    DTRACEIF( PHYDISP_DEBUG, "P%u%s notified mode %u is out of range (v %zd)",
...
uint32_t PhysicalDisplay::getAppliedWidth( void  ) const
{
    ...
    DTRACEIF( PHYDISP_DEBUG, "P%u%s applied mode %u is out of range (v %zd)",
...
{
    DTRACEIF( PHYDISP_DEBUG || MODE_DEBUG, "PhysicalDisplay::onGetDisplayAttribute config handle:%x, attribute:%d", configHandle, attribute );
    ...
    HWCASSERT( configHandle >= CONFIG_HANDLE_BASE );
    ...
    default:
        *pValue = 0;
	ETRACE("PhysicalDisplay::onGetDisplayAttribute: UNKNOWN ATTRIBUTE %d", attribute);
        break;
    }

    DTRACEIF( PHYDISP_DEBUG || MODE_DEBUG, "PhysicalDisplay::onGetDisplayAttribute: %s = %d",
...
{
    DTRACEIF( PHYDISP_DEBUG || MODE_DEBUG, "PhysicalDisplay::onSetActiveConfig config:%x", configIndex );
...
void PhysicalDisplay::createSoftwareVSyncGeneration( void )
{
	...
	mpSoftwareVsyncThread.reset(new SoftwareVsyncThread(mDevice, this, initialPeriod ));
        if ( mpSoftwareVsyncThread == NULL )
        {
...
{
    ETRACEIF( mpSoftwareVsyncThread == NULL, "HWC:P%u Software vsync thread not created", getDisplayManagerIndex() );
    if ( mbSoftwareVSyncEnabled )
        return;
    ATRACE_INT_IF( VSYNC_DEBUG, HWCString::format( "HWC:P%u SW VSYNC", getDisplayManagerIndex() ).string(), 1 );
...
void PhysicalDisplay::disableSoftwareVSyncGeneration( void )
{
    ...
    ATRACE_INT_IF( VSYNC_DEBUG, HWCString::format( "HWC:P%u SW VSYNC", getDisplayManagerIndex() ).string(), 0 );
...
void PhysicalDisplay::processDynamicDisplayTimings( void )
{
    ...
    {
	const Timing& tt = mDisplayTimings.at(t);
	    ...
	    const Timing& tm = mDisplayTimings.at(m);
        ...
        {
	    DTRACEIF(MODE_DEBUG, "Display processDynamicDisplayTimings %s", tt.dump().string());
        }
        else
        {
	    ...
	    mDisplayTimings.insert(mDisplayTimings.begin() + t, nt);
	    DTRACEIF(MODE_DEBUG, "Display processDynamicDisplayTimings %s", nt.dump().string());
        }
    }
}
...
void PhysicalDisplay::setRequestedTiming( uint32_t timingIndex )
{
    HWC_ASSERT_LOCK_NOT_HELD2(mTimingLock);
    ScopedSpinLock _l( mTimingLock );
    HWCASSERT( timingIndex != UnknownDisplayTiming );
...
void PhysicalDisplay::notifyNewRequestedTiming( void )
{
    ...
    {
	HWCASSERT( mRequestedTimingIndex != UnknownDisplayTiming );
        Timing t;
...
bool PhysicalDisplay::haveNotifiedTimingChange( uint32_t& timingIndex )
{
    ...
    if ( mbNotifiedTiming )
    {
	HWCASSERT( mNotifiedTimingIndex != UnknownDisplayTiming );
...
HWCString PhysicalDisplay::dump( void ) const
{
    HWCString str;
    str = HWCString::format( "RPD:%u %8s : PhysSize:%ux%umm Timing:Requested 0x%x Notified 0x%x [Res:%ux%u, Period:%uus] Applied 0x%x",
