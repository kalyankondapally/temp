...
#ifndef COMMON_DISPLAY_DISPLAYQUEUE_H
#define COMMON_DISPLAY_DISPLAYQUEUE_H
...
public:
	...
	HWCString dump( void ) const
        {
	    return mbValid ? HWCString::format( "frame:%u %" PRIi64 "s %03" PRIi64 "ms [timeline:%u]",
                             mHwcIndex,
                             mHwcReceivedTime/1000000000, (mHwcReceivedTime%1000000000)/1000000,
                             mTimelineIndex )
			   : HWCString::format( "<no valid frameId>" );
        }
	...
	virtual HWCString dump( void ) const;
    ...
    private:
        Layer mLayer;
        int mAcquireFence;
	std::shared_ptr<AbstractBufferManager::Buffer> mpAcquiredBuffer;
        bool mbSet:1;
    };
    ...
    void init( const HWCString& threadName );

    // Get DisplayQueue thread name.
    HWCString getName( void ) { return mName; }
    ...
    // Get HWC context.
    // Classes deriving from FrameQueue must implement this.
    virtual GpuDevice& getGpuDevice( void ) = 0;
...
protected:
    ...
    // Worker thread.
    class Worker : public HWCThread
    {
    public:
	Worker( DisplayQueue& queue, const HWCString& threadName );
        virtual ~Worker( );
    ...
    std::unique_ptr<Worker>              mpWorker;
    ...
    std::condition_variable mConditionWorkConsumed;

    // Condition used to signal that a presented frame has been released.
    std::condition_variable mConditionFrameReleased;
    ...
    std::thread::id getWorkerTid( void );
};

}; // namespace hwcomposer

#endif // COMMON_DISPLAY_DISPLAYQUEUE_H
