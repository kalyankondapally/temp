...
#ifndef COMMON_DISPLAY_DISPLAY_CAPS_H
#define COMMON_DISPLAY_DISPLAY_CAPS_H
...
public:
    ...
    class PlaneCaps
    {
	    ...
	    void setDisplayFormats( const std::vector<int32_t>& formats );
	    ...
	    int32_t     getCSCFormat(ECSCClass format) const    { HWCASSERT(format < CSC_CLASS_MAX); return mCSCFormat[format]; }
	    ...
	    HWCString transformLUTString( void ) const;

            // Get display-format look-up-table as human-readable string.
	    HWCString displayFormatLUTString( void ) const;

            // Get csc format look-up-table as human-readable string.
	    HWCString cscFormatLUTString( void ) const;
	    ...
	    std::vector<ETransform> mTransformLUT;       //< Supported transforms.
	    std::vector<int32_t> mDisplayFormatLUT;      //< Supported display formats.
    ...
    class GlobalScalingCaps
    {
	    ...
	    HWCString getFlagsString() const;
    ...
    const PlaneCaps& getPlaneCaps( uint32_t plane ) const   { return *mpPlaneCaps.at( plane ); }

    // Accessor for non-const plane caps.
    PlaneCaps& editPlaneCaps( uint32_t plane )              { return *mpPlaneCaps.at( plane ); }

    // Add a plane to the caps
    void add(PlaneCaps* pPlane)                             { mpPlaneCaps.emplace_back(pPlane); }
    ...
    const ZOrderLUTEntry* getZOrderLUT( void ) const        { return mZOrderLUT.data( ); }

    // Get count of entries in the ZOrder look-up-table.
    uint32_t getNumZOrders( void ) const                    { return mZOrderLUT.size( ); }

    // set the zorder LUT
    void setZOrderLUT(std::vector<ZOrderLUTEntry>& lut)
    {
        mZOrderLUT = lut;
        updateZOrderMasks();
    }
    ...
    bool isOpaqueControlSupported( uint32_t plane ) const                           { return mpPlaneCaps.at(plane)->isOpaqueControlSupported( ); }
    bool isBlendingSupported( uint32_t plane ) const                                { return mpPlaneCaps.at(plane)->isBlendingSupported( ); }
    bool isScalingSupported( uint32_t plane ) const                                 { return mpPlaneCaps.at(plane)->isScalingSupported( ); }
    bool isDecryptSupported( uint32_t plane ) const                                 { return mpPlaneCaps.at(plane)->isDecryptSupported( ); }
    bool isWindowingSupported( uint32_t plane ) const                               { return mpPlaneCaps.at(plane)->isWindowingSupported( ); }
    bool isSourceOffsetSupported( uint32_t plane ) const                            { return mpPlaneCaps.at(plane)->isSourceOffsetSupported( ); }
    bool isSourceCropSupported( uint32_t plane )const                               { return mpPlaneCaps.at(plane)->isSourceCropSupported( ); }
    bool isDisableSupported( uint32_t plane ) const                                 { return mpPlaneCaps.at(plane)->isDisableSupported( ); }
    bool isTransformSupported( uint32_t plane, ETransform transform ) const         { return mpPlaneCaps.at(plane)->isTransformSupported( transform ); }
    bool isDisplayFormatSupported( uint32_t plane, int32_t displayFormat ) const    { return mpPlaneCaps.at(plane)->isDisplayFormatSupported( displayFormat ); }
    uint32_t getZOrderPreMask( uint32_t plane ) const                               { return mpPlaneCaps.at(plane)-> getZOrderPreMask(); }
    uint32_t getZOrderPostMask( uint32_t plane ) const                              { return mpPlaneCaps.at(plane)-> getZOrderPostMask(); }
    bool isSourceSizeSupported( uint32_t plane, uint32_t width, uint32_t height, uint32_t pitch ) const
    {
	return mpPlaneCaps.at(plane)->isSourceSizeSupported( width, height, pitch );
    }
    ...
    // Get plane capabilities as human-readable string.
    HWCString planeCapsString( uint32_t plane ) const
	{ return mpPlaneCaps.at(plane)->capsString( ); }

    // Get plane transform look-up-table as human-readable string.
    HWCString planeTransformLUTString( uint32_t plane ) const
	{ return mpPlaneCaps.at(plane)->transformLUTString( ); }

    // Get plane display-format look-up-table as human-readable string.
    HWCString planeDisplayFormatLUTString( uint32_t plane ) const
	{ return mpPlaneCaps.at(plane)->displayFormatLUTString( ); }

    // Get plane color space conversion format look-up-table as human-readable string.
    HWCString planeCscFormatLUTString( uint32_t plane ) const
	{ return mpPlaneCaps.at(plane)->cscFormatLUTString( ); }

    // Get zorders as human-readable string.
    HWCString zOrdersString( void ) const;

    // Get display capabilities as human-readable string.
    virtual HWCString displayCapsString( void ) const;

    static ECSCClass halFormatToCSCClass( int32_t halFmt, bool bForceOpaque = false );

    HWCString globalScalingCapsString( ) const
...
protected:
    ...
    std::vector<PlaneCaps*>      mpPlaneCaps;            // Plane capabilities.
    GlobalScalingCaps            mGlobalScalingCaps;     // Display capabilities for global scaling.
    std::vector<ZOrderLUTEntry>  mZOrderLUT;             // The ZOrder look-up-table.
...
#endif // COMMON_DISPLAY_DISPLAY_CAPS_H
