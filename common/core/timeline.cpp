    ...
    mNextFutureTime( 0 )
{
  ...
  mSyncTimeline = open("/dev/sw_sync", O_RDWR);
#else
  mSyncTimeline = open("/sys/kernel/debug/sync/sw_sync", O_RDWR);
#endif
    ETRACEIF( mSyncTimeline == -1, "Failed to create sync timeline." );
    DTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline %d(%s) [mCurrentTime %u/mNextFutureTime %u] created",
        mSyncTimeline, mName.string( ), mCurrentTime, mNextFutureTime );
}

Timeline::~Timeline( )
{
    DTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline %d(%s) [mCurrentTime %u/mNextFutureTime %u] destroy",
        mSyncTimeline, mName.string( ), mCurrentTime, mNextFutureTime );
    uninit( );
    if ( mSyncTimeline != -1 )
    {
...
bool Timeline::init( String8 name, uint32_t firstFutureTime )
{
    if ( mSyncTimeline == -1 )
    {
       ETRACE( "Missing sync timeline." );
        return false;
    }
    mName = name;
    DTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline %d(%s) [mCurrentTime %u/mNextFutureTime %u] init firstFutureTime %u",
        mSyncTimeline, mName.string( ), mCurrentTime, mNextFutureTime, firstFutureTime );
    ETRACEIF( !firstFutureTime, "Expected non-zero firstFutureTime" );
    if ( mCurrentTime != mNextFutureTime )
    {
        advanceTo( mNextFutureTime );
    }
    mNextFutureTime = mCurrentTime + firstFutureTime;
    DTRACEIF( SYNC_FENCE_DEBUG, " == mCurrentTime %u/mNextFutureTime %u", mCurrentTime, mNextFutureTime );
    return true;
}

void Timeline::uninit( void )
{
    DTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline %d(%s) [mCurrentTime %u/mNextFutureTime %u] uninit",
        mSyncTimeline, mName.string( ), mCurrentTime, mNextFutureTime );
    if ( mCurrentTime != mNextFutureTime )
    {
        advanceTo( mNextFutureTime );
...
{
    DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:alloc fence" );

    if ( mSyncTimeline == -1 )
    {
	WTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline is not initialised" );
    ...
    if ( newFence < 0 )
    {
       ETRACE( "Timeline %d : Failed to alloc new fence [%s] : %s", mSyncTimeline, fenceName, strerror(errno) );
        Log::alogd( true, "NativeFence: create !ERROR!" );
        return NullNativeFence;
    }

    DTRACEIF( SYNC_FENCE_DEBUG, "SyncTimeline %d(%s) : Allocated new fence %d(%s)",
...
{
    HWCASSERT( pTimelineIndex != NULL );

    DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:create fence" );
...
Timeline::NativeFence Timeline::repeatFence( uint32_t* pTimelineIndex )
{
    ...
    DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:repeat fence" );
...
{
    HWCASSERT( pFence && pOtherFence );
    DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:merge fence %p/%d other %p/%d", pFence, *pFence, pOtherFence, *pOtherFence );
	...
	// FIXME:
	struct sync_fence_info_data* pInfo1 = NULL;//sync_fence_info( *pFence );
	struct sync_fence_info_data* pInfo2 = NULL;//sync_fence_info( *pOtherFence );

        if ( pInfo1 )
        {
	    DTRACEIF( SYNC_FENCE_DEBUG && pInfo1, "NativeFence Info1: %s status %d", pInfo1->name, pInfo1->status );
            struct sync_pt_info* pSyncPointInfo = NULL;
	    // FIXME:
	   /* while ( ( pSyncPointInfo = sync_pt_info( pInfo1, pSyncPointInfo ) ) != NULL )
            {
		DTRACEIF( SYNC_FENCE_DEBUG, "  SyncPoint Driver %s Status %d Timestamp %.03f",
        ...
        if ( pInfo2 )
        {
	    DTRACEIF( SYNC_FENCE_DEBUG && pInfo2, "NativeFence Info2: %s status %d", pInfo2->name, pInfo2->status );
            struct sync_pt_info* pSyncPointInfo = NULL;
	    // FIXME:
	...
	DTRACEIF( SYNC_FENCE_DEBUG, "Timeline : Merged fence %d(%s)", mergedFence, fenceName );
        close( *pFence );
        close( *pOtherFence );
        *pFence = mergedFence;
        *pOtherFence = NullNativeFence;
...
{
    DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:dup fence %d", *pOtherFence );
    if ( *pOtherFence < 0 )
    {
        if ( Log::wantLog( SYNC_FENCE_DEBUG ) )
        {
    ...
    if ( newFence < 0 )
    {
       ETRACE( "Failed to dup fence : %s", strerror(errno) );
...
void Timeline::advance( uint32_t ticks )
{
    ...
    {
       ETRACE( "**************** CRITICAL ****************" );
       ETRACE( "Failed to advance sync timeline %d(%s)", mSyncTimeline, mName.string( ) );
       ETRACE( "**************** CRITICAL ****************" );
    }
}
...
void Timeline::advanceTo( uint32_t absSync )
{
    ...
    if ( delta > 0 )
    {
	DTRACEIF( SYNC_FENCE_DEBUG, "advanceTo( %u ) mCurrentTime %u => delta %u", absSync, mCurrentTime, delta );
...
void Timeline::dumpTimeline( const char* pchPrefix ) const
{
    if (SYNC_FENCE_DEBUG)
    {
	DTRACE( "%s%sSyncTimeline %d(%s) mNextFutureTime %u",
            pchPrefix ? pchPrefix : "",
            pchPrefix ? " - " : "",
            mSyncTimeline,
            mName.string( ),
...
void Timeline::logFence( const NativeFence* pFence, const char* pchPrefix )
{
    if (SYNC_FENCE_DEBUG)
    {
	DTRACE( "%s - %s", pchPrefix, dumpFence( pFence ).string() );
    }
}
...
#ifndef USE_ANDROID_SYNC
int Timeline::sw_sync_fence_create(int fd, const char *name, unsigned value) {
...
int Timeline::sw_sync_timeline_inc(int fd, unsigned count) {
  uint32_t arg = count;
