...
#ifndef COMMON_CORE_TIMELINE_H
#define COMMON_CORE_TIMELINE_H
...
public:
    ...
    public:
        ...
        {
	    HWCASSERT( mBoundFences == 0, "%s mBoundFences %d", __FUNCTION__, mBoundFences );
            mFence = fence;
            mbSignalled = false;
            if ( fence >= 0 )
            {
        ...
        void cancel( void )
        {
	    HWCASSERT( mBoundFences >= 1, "%s mBoundFences %d", __FUNCTION__, mBoundFences );
	    HWCASSERT( !mbSignalled, "%s mbSignalled %d", __FUNCTION__, mbSignalled );
	    ...
	    HWCASSERT( mBoundFences >= 0, "%s mBoundFences %d", __FUNCTION__, mBoundFences );
	    HWCString prefix;
	    int fence = mBoundFences;
	    prefix = HWCString::format( "%s - Refs:%d",  pchPrefix, fence );
	...
	HWCString dump( void ) const
	    ...
	    int fence = mBoundFences;
	    HWCString str = HWCString::format( "H[ Refs:%d/%c ", fence, mbSignalled? 'S' : 'B' )
                        + Timeline::dumpFence( &mFence )
			+ HWCString::format( " %s ]", ( fence && !mbSignalled ) ? "BLOCKED" : "NON-BLOCKED" );
	    HWCASSERT( fence >= 0, "%s %s mBoundFences %d", __FUNCTION__, str.string(), fence );
            return str;
        }
    ...
    protected:
        ...
        {
	    HWCASSERT( mBoundFences >= 0, "%s mBoundFences %d", __FUNCTION__, mBoundFences );
	    std::atomic_fetch_add( &mBoundFences, 1 );
        }

        // Decrement count of bound references.
        void decBoundFences( void )
        {
	    HWCASSERT( mBoundFences >= 1, "%s BoundFences %u", __FUNCTION__, mBoundFences );
	    std::atomic_fetch_sub( &mBoundFences, 1 );
        }
	...
	std::atomic<int> mBoundFences;  //< A count of sync points linked to this fence.
    ...
    public:
        ...
        {
	    HWCASSERT( ( meType == eTypeNative ) || ( meType == eTypeUnspecified ) );
        ...
        {
	    HWCASSERT( ( meType == eTypeHwc ) || ( meType == eTypeUnspecified ) );
            return mpHwcFence;
        }
        ...
        {
	    HWCASSERT( ( fence == NullNativeFence ) || ( meType != eTypeUnspecified ) );
            switch ( meType )
            {
                case eTypeNative:
		    HWCASSERT( mpNativeFence );
                    Timeline::closeFence( mpNativeFence );
                    *mpNativeFence = fence;
                    return;
                case eTypeHwc:
		    HWCASSERT( mpHwcFence );
                    mpHwcFence->close( );
	...
	HWCString dump( void ) const
        {
            switch ( meType )
            {
                case eTypeNative:
		    HWCASSERT( mpNativeFence );
		    return HWCString::format( "FenceReference %p [ ", this )
    ...
    bool init( HWCString name, uint32_t firstFutureTime = 1 );
	...
	HWCASSERT( pFence );
	HWCASSERT( timeoutMs > 0 );
	DTRACEIF( SYNC_FENCE_DEBUG, "NativeFence: wait fence %p/%d", pFence, *pFence );
    ...
    static bool checkAndClose( NativeFence *pFence )
    {
        int err = 0;
	HWCASSERT( pFence );
	DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:checkAndClose fence %p/%d", pFence, *pFence );
        // A negative fence is considered signalled
        if (*pFence >= 0)
        {
            // Any error should be considered as not signalled
    ...
    static bool check( NativeFence *pFence )
    {
        int err = 0;
	HWCASSERT( pFence );
	DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:check fence %p/%d", pFence, *pFence );
        // A negative fence is considered signalled
        if (*pFence >= 0)
        {
            // Any error should be considered as not signalled
    ...
    {
	HWCASSERT( pFence );
	DTRACEIF( SYNC_FENCE_DEBUG, "Timeline:close fence %p/%d", pFence, *pFence );
        if (*pFence >= 0)
        {
            if ( Log::wantLog( ) )
            {
    ...
    static HWCString dumpFence( const NativeFence* pFence );
