...
public:
    ...
    {
	HWCASSERT(mNumProps < MAX_PROPERTIES);
        mProps[mNumProps] = id;
    ...
    mDrm( Drm::get() )
{
    ...
    HWCASSERT(mPropFb   != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropDstX != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropDstY != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropDstW != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropDstH != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropSrcX != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropSrcY != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropSrcW != Drm::INVALID_PROPERTY);
    HWCASSERT(mPropSrcH != Drm::INVALID_PROPERTY);
...
int DrmNuclearHelper::setCrtcNuclear( const drmModeModeInfoPtr pModeInfo, const Layer* pLayer )
{
    ...
    if (pModeInfo)
    {
	pModeBlob.reset(mDrm.createBlob( pModeInfo, sizeof(drmModeModeInfo) ));
        if (pModeBlob == NULL)
        {
