...
DrmUEventThread::DrmUEventThread(GpuDevice& device, Drm& drm) : HWCThread(0, "hwc.uevent"),
    mDevice(device),
    mDrm(drm),
    mESDConnectorType(-1),
    mESDConnectorID(-1),
...
bool DrmUEventThread::Initialize()
{
    ...
    if (mUeventFd < 0) {
	ETRACE("failed to create uevent socket, %d", errno);
	return false;
    }
    ...
    WTRACEIF(HPLUG_DEBUG && err, "setsockopt failed: %d", err);

    if (bind(mUeventFd, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
	ETRACE("failed to bind uevent socket, %d", errno);
        close(mUeventFd);
	return false;
    }
...
void DrmUEventThread::HandleRoutine()
{
    struct pollfd fds;
    fds.fd = mUeventFd;
    fds.events = POLLIN;
...
Drm::UEvent DrmUEventThread::decodeUEvent( void )
{
#if HPLUG_DEBUG
    {
        ...
        while ( pchLine < pchEnd )
        {
            ...
            if ( !lineChars )
                break;
	    DTRACE( "decodeHotPlugEvent [%s]", pchLine );
            pchLine += lineChars + 1;
        }
    }
#endif
    ...
    while ( fieldsFound < requiredFieldCount && pchLine < pchEnd )
    {
        ...
        {
	    DTRACEIF( HPLUG_DEBUG, "decodeHotPlugEvent return UEVENT_UNRECOGNISED %s != %s", pchLine, requiredFields[fieldsFound]);
    ...
    if ( fieldsFound == requiredFieldCount && pchLine < pchEnd )
    {
        ...
        {
	    DTRACEIF( HPLUG_DEBUG, "decodeUEvent return HOTPLUG_CHANGED - %s", pchLine );
            return Drm::UEvent::HOTPLUG_CHANGED;
        }
        else if (strncmp(pchLine, UEVENT_HOTPLUG_IMMINENT, sizeof(UEVENT_HOTPLUG_IMMINENT)) == 0)
        {
	    DTRACEIF( HPLUG_DEBUG, "decodeUEvent return HOTPLUG_IMMINENT - %s", pchLine );
        ...
        {
	    DTRACEIF( HPLUG_DEBUG, "decodeUEvent return UEVENT_ESD_RECOVERY - %s", pchLine );
            pchLine += sizeof(UEVENT_I915_DISPLAY_RESET_DSI) + strlen(UEVENT_I915_CONNECTOR_ID);
	    DTRACEIF( HPLUG_DEBUG, "  From connectorID: %s", pchLine );
    ...
    DTRACEIF( HPLUG_DEBUG, "decodeUEvent return UEVENT_UNRECOGNISED - %s", pchLine);
    return Drm::UEvent::UNRECOGNISED;
}
...
status_t DrmUEventThread::onUEvent( void )
{
    ...
    mDevice.EnsureCompositorIsReady();
