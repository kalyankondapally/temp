...
typedef struct _drmModeModeInfo *drmModeModeInfoPtr;
typedef struct _drmModeEncoder *drmModeEncoderPtr;
typedef struct _drmModeConnector *drmModeConnectorPtr;
typedef struct _drmModePlaneRes *drmModePlaneResPtr;
...
#define HWC_USE_ATOMIC_NUCLEAR                 1
#else
#define HWC_USE_ATOMIC_NUCLEAR                 0
#endif
...
#define DEFAULT_DISPLAY_GRALLOCFORMAT               DRM_FORMAT_ABGR8888
...
public:
    ...
    class Blob {
        public:
	    static Blob* create( Drm& drm, const void* pData, uint32_t size );
            ~Blob();
    ...
    // Create proprerty blob. Ownership passed to caller
    Blob* createBlob( const void* pData, uint32_t size );
   ...
   private:
    ...
    GpuDevice*                      mDevice;                              // Established on probe().
    ...
    std::unique_ptr<DrmEventThread>              mpEventThread;
    std::unique_ptr<DrmUEventThread>             mpUEventThread;
