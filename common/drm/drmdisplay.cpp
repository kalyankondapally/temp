...
#define DRMDISPLAY_ASSERT_PRODUCER_THREAD \
    DTRACEIF( DRM_DISPLAY_DEBUG, "ASSERT PRODUCER: Worker:%u This:%u", getWorkerTid(), gettid() ); \
    HWCASSERT( gettid() != getWorkerTid() );
#define DRMDISPLAY_ASSERT_CONSUMER_THREAD \
    DTRACEIF( DRM_DISPLAY_DEBUG, "ASSERT CONSUMER: Worker:%u This:%u", getWorkerTid(), gettid() ); \
    HWCASSERT( ( getWorkerTid() == 0 ) || ( gettid() == getWorkerTid() ) );
#define DRMDISPLAY_ASSERT_EXTERNAL_THREAD \
    DTRACEIF( DRM_DISPLAY_DEBUG, "ASSERT EXTERNAL: Worker:%u This:%u", getWorkerTid(), gettid() ); \
    HWCASSERT( gettid() != getWorkerTid() );
#else
#define DRMDISPLAY_ASSERT_PRODUCER_THREAD
#define DRMDISPLAY_ASSERT_CONSUMER_THREAD
...
DrmDisplay::DrmDisplay( GpuDevice& device, uint32_t drmConnectorIndex ) :
    PhysicalDisplay(device),
...
status_t DrmDisplay::open(drmModeConnector *pConnector, bool bRegisterWithHwc)
{
    ...
    if (pEncoder == NULL)
    {
	ETRACE("Failed to get encoder for connector, skipping");
    ...
    ETRACEIF( mPropDPMS == Drm::INVALID_PROPERTY, "Failed to get DPMS property ID" );
                                       ...
                                       uint32_t dstW, uint32_t dstH )
{
    ...
    DTRACEIF( GLOBAL_SCALING_DEBUG, "DrmDisplay::acquireGlobalScaling " DRMDISPLAY_ID_STR " %s "
...
{
    HWCASSERT( getDisplayCaps().getGlobalScalingCaps().getFlags()
...
void DrmDisplay::issueGlobalScalingConfig( drm_mode_set_display& display, const SGlobalScalingConfig& globalScalingNew )
{
    ...
    DTRACEIF( GLOBAL_SCALING_DEBUG, "DrmDisplay::issueGlobalScalingConfig " DRMDISPLAY_ID_STR "/%s, globalScalingNew.mbEnabled:%d, mGlobalScalingActive.mbEnabled:%d",
...
void DrmDisplay::finalizeGlobalScalingConfig( const SGlobalScalingConfig& globalScalingNew )
{
    ...
    if ( mGlobalScalingActive.mbEnabled && !globalScalingNew.mbEnabled )
    {
	    ...
	    DTRACEIF( GLOBAL_SCALING_DEBUG, DRMDISPLAY_ID_STR " Panel fitter released.", DRMDISPLAY_ID_PARAMS );
        }
        else
        {
	    DTRACEIF( GLOBAL_SCALING_DEBUG, DRMDISPLAY_ID_STR " Panel fitter 'do not release'.", DRMDISPLAY_ID_PARAMS );
        }
    }
                                 ...
                                 uint32_t dstW, uint32_t dstH )
{
    DRMDISPLAY_ASSERT_CONSUMER_THREAD
#if VPG_DRM_HAVE_PANEL_FITTER
    HWCASSERT( mPropPanelFitterMode != Drm::INVALID_PROPERTY );
    HWCASSERT( mPropPanelFitterSource != Drm::INVALID_PROPERTY );
    if ( !mDrm.isPanelFitterAcquired( mDrmConnectorID ) )
    {
	ETRACE( DRMDISPLAY_ID_STR " Can not enable panel fitter - not acquired.", DRMDISPLAY_ID_PARAMS );
        return false;
    }
    if ( mDrm.setPanelFitterSourceSizeProperty( mDrmConnectorID,
                                                ...
                                                srcW, srcH ) )
    {
	ETRACE( DRMDISPLAY_ID_STR " Set panel fitter source size property failed.", DRMDISPLAY_ID_PARAMS );
        return false;
    }
    if ( ( mDrmPanelFitterMode < 0 ) || ((uint32_t)mDrmPanelFitterMode != pfitMode ) )
    {
	DTRACEIF( GLOBAL_SCALING_DEBUG, DRMDISPLAY_ID_STR " Set PFIT Mode : %u", DRMDISPLAY_ID_PARAMS, pfitMode );
        if ( mDrm.setPanelFitterProperty( mDrmConnectorID,
                                          ...
                                          dstW, dstH ))
        {
	    ETRACE( DRMDISPLAY_ID_STR " Set panel fitter property failed.", DRMDISPLAY_ID_PARAMS );
            return false;
        }
...
bool DrmDisplay::resetPanelFitter( void )
{
    ...
    if ( mDrm.isPanelFitterAcquired( mDrmConnectorID ) )
    {
	DTRACEIF( GLOBAL_SCALING_DEBUG, DRMDISPLAY_ID_STR " Reset PFIT Mode : %u", DRMDISPLAY_ID_PARAMS, DRM_PFIT_OFF );
        if ( mDrm.setPanelFitterSourceSizeProperty( mDrmConnectorID,
                                                    mPropPanelFitterSource,
...
bool DrmDisplay::applyGlobalScalingConfig( const SGlobalScalingConfig& globalScalingNew )
{
    ...
    DTRACEIF( GLOBAL_SCALING_DEBUG, "DrmDisplay::applyGlobalScalingConfig " DRMDISPLAY_ID_STR "/%s, globalScalingNew.mbEnabled:%d, mGlobalScalingActive.mbEnabled:%d",
                    DRMDISPLAY_ID_PARAMS, getConnectionDesc().string(), globalScalingNew.mbEnabled, mGlobalScalingActive.mbEnabled );
    if ( globalScalingNew.mbEnabled )
    {
        if( ( !mGlobalScalingActive.mbEnabled) ||// currently not enabled
...
void DrmDisplay::onSet(const Content::Display& display, uint32_t zorder, int* pRetireFenceFd)
{
    ...
    DTRACEIF( DRM_DEBUG, "DrmDisplay::onSet P%u zorder:%d %s", getDisplayManagerIndex(), zorder, display.dump().string());
	...
	HWCASSERT( mGlobalScalingRequested.mSrcH == display.getHeight() );
	HWCASSERT( mGlobalScalingRequested.mDstX == dst.left );
	HWCASSERT( mGlobalScalingRequested.mDstY == dst.top );
	HWCASSERT( mGlobalScalingRequested.mDstW == (uint32_t)(dst.right - dst.left) );
	HWCASSERT( mGlobalScalingRequested.mDstH == (uint32_t)(dst.bottom - dst.top) );
    }
    else
    {
	HWCASSERT( !mGlobalScalingRequested.mbEnabled );
...
int DrmDisplay::onBlank(bool bEnable, bool bIsSurfaceFlinger)
{
    ...
    DTRACEIF( DRMDISPLAY_MODE_DEBUG, "%s%s", bEnable ? "Blank" : "Unblank", bIsSurfaceFlinger ? " (SF)" : "" );
...
void DrmDisplay::allocateBlankingLayer( uint32_t width, uint32_t height )
{
    ...
    if ( !width && !height )
    {
	    ...
	    DTRACEIF( DRM_DEBUG || GLOBAL_SCALING_DEBUG,
      ...
      || ( mBlankLayer.getBufferHeight() != height ) )
    {
        ...
        {
	    ETRACE( "Can't allocate blanking (mode %ux%u)", width, height );
        }
    }
    mBlankBufferFramesSinceLastUsed = 0;
}
...
{
    DTRACEIF( DRM_DEBUG, "DRMDisplay " DRMDISPLAY_ID_STR " dropAllFrames( )", DRMDISPLAY_ID_PARAMS );
    DisplayQueue::dropAllFrames();
}

void DrmDisplay::flush( uint32_t frameIndex, nsecs_t timeoutNs )
{
    DTRACEIF( DRM_DEBUG, "DRMDisplay " DRMDISPLAY_ID_STR " flush( Frame:%u, Timeout:%" PRIi64 ")",
...
void DrmDisplay::setCurrentConnectionModes( drmModeConnector* pNewConnector )
{
    ...
    DTRACEIF( HPLUG_DEBUG, DRMDISPLAY_ID_STR " HotPlug modes x%d", DRMDISPLAY_ID_PARAMS, pNewConnector->count_modes );
    for ( int32_t i = 0; i < pNewConnector->count_modes; i++ )
    {
        drmModeModeInfo& m = pNewConnector->modes[i];
	DTRACEIF( HPLUG_DEBUG, DRMDISPLAY_ID_STR " HotPlug mode %d : %s", DRMDISPLAY_ID_PARAMS, i, Drm::modeInfoToString( m ).string() );
...
{
    DTRACEIF( HPLUG_DEBUG, DRMDISPLAY_ID_STR " HotPlug check timings x%d", DRMDISPLAY_ID_PARAMS, pNewConnector->count_modes );
    if ( pNewConnector->count_modes != (int32_t)mCurrentConnectionModes.size() )
    {
	DTRACEIF( HPLUG_DEBUG, DRMDISPLAY_ID_STR " HotPlug (now) x%u != (was) x%zu",
    ...
    for ( int32_t i = 0; i < pNewConnector->count_modes; i++ )
    {
        ...
        {
	    DTRACEIF( HPLUG_DEBUG, DRMDISPLAY_ID_STR " HotPlug timing %d : (now) %s != (was) %s",
...
void DrmDisplay::issueHotPlug( void )
{
    ...
    HWCASSERT( mCurrentConnection.isConnected() );
...
void DrmDisplay::reconnect( void )
{
    ...
    {
	DTRACE("It has been disconnected, not to reconnect.");
        return;
    }
...
{
    DTRACEIF( MODE_DEBUG, "DrmDisplay setAppliedTiming timing index %u", timingIndex );
...
void DrmDisplay::doSetDisplayMode(uint32_t mode)
{
    ...
    HWCASSERT( bOK );
    HWCASSERT( mode < mTimingToConnectorMode.size() );
    ...
    if ( status != Drm::SUCCESS )
    {
	ETRACE("" DRMDISPLAY_ID_STR " set mode - failed to set video mode %d: %s",
...
bool DrmDisplay::getSeamlessMode( drmModeModeInfo &modeInfoOut )
{
	...
	HWCASSERT( (uint32_t)getDrmConnector()->count_modes >= appliedMode );
...
void DrmDisplay::legacySeamlessAdaptMode( const Layer* pLayer )
{
    ...
    {
	DTRACE( "Setting video mode for Crtc %d, Display " DRMDISPLAY_ID_STR ", now %uHz",
...
bool DrmDisplay::defaultFrameRequired( void )
{
    ...
    {
	WTRACE( "eDF_Auto not implemented" );
...
void DrmDisplay::setDisplay( int32_t overrideMode )
{
    ...
    {
	DTRACE( DRMDISPLAY_ID_STR " Already initialised", DRMDISPLAY_ID_PARAMS );
        return;
    }
    ...
    DTRACEIF( DRMDISPLAY_MODE_DEBUG, DRMDISPLAY_ID_STR " Setting Drm Mode  (DPMS -> DRM_MODE_DPMS_ON)", DRMDISPLAY_ID_PARAMS );
    mDrm.setDPMSProperty( mDrmConnectorID, mPropDPMS, DRM_MODE_DPMS_ON );
...
void DrmDisplay::resetDisplay( void )
{
    ...
    if ( meStatus != AVAILABLE )
    {
	DTRACE( DRMDISPLAY_ID_STR " Already suspended", DRMDISPLAY_ID_PARAMS );
        return;
    }
...
bool DrmDisplay::setNewConnection( Connection& newConnection )
{
    ...
    if ( !pCrtc )
    {
	ETRACE( "Display start - get Crtc error [CrtcID %u]", newConnection.getCrtcID() );
...
void DrmDisplay::consumeShutdown( uint32_t timelineIndex )
{
    ...
    // Disable DPMS.
    DTRACEIF( DRMDISPLAY_MODE_DEBUG, DRMDISPLAY_ID_STR " Setting Drm Mode  (DPMS -> DRM_MODE_DPMS_OFF)", DRMDISPLAY_ID_PARAMS );
    mDrm.setDPMSProperty( mDrmConnectorID, mPropDPMS, DRM_MODE_DPMS_OFF );
    ...
    if ( status != Drm::SUCCESS )
    {
	ETRACE("Failed to reset video mode for Crtc %d, Display " DRMDISPLAY_ID_STR ", %d: %s",
...
void DrmDisplay::consumeSuspend( uint32_t timelineIndex, bool bUseDPMS, bool bDeactivateDisplay )
{
	...
	Log::DTRACE( DRM_SUSPEND_DEBUG, DRMDISPLAY_ID_STR " -> SUSPENDED", DRMDISPLAY_ID_PARAMS );
        ...
        if ( mbScreenCtlOn )
        {
	    DTRACEIF( DRM_BLANKING_DEBUG, DRMDISPLAY_ID_STR " screen ctl = 0", DRMDISPLAY_ID_PARAMS );
	    ...
	    Log::DTRACE( DRM_SUSPEND_DEBUG, DRMDISPLAY_ID_STR " SUSPENDED DPMS_OFF", DRMDISPLAY_ID_PARAMS );
            ...
            for (;;)
            {
                if ( !bOK )
                {
		    WTRACE( "DRM_MODE_DPMS_ASYNC_OFF did not complete - forcing DRM_MODE_DPMS_OFF" );
                    mDrm.setDPMSProperty( mDrmConnectorID, mPropDPMS, DRM_MODE_DPMS_OFF );
                    break;
                }
                usleep( wait25Ms );
...
void DrmDisplay::consumeResume( void )
{
	...
	Log::DTRACE( DRM_SUSPEND_DEBUG,
        ...
        {
	    ETRACE( DRMDISPLAY_ID_STR " Can not resume display [isConnected:%d hasPipe:%d]",
		...
		DTRACEIF( DRM_BLANKING_DEBUG, DRMDISPLAY_ID_STR "Synchronizing pre-resume", DRMDISPLAY_ID_PARAMS );
                synchronizeFromConsumer();
		DTRACEIF( DRM_BLANKING_DEBUG, DRMDISPLAY_ID_STR "Synchronizing pre-resume complete", DRMDISPLAY_ID_PARAMS );
            }
            mbSuspendDeactivated = false;
        }
	...
	Log::DTRACE( DRM_SUSPEND_DEBUG, DRMDISPLAY_ID_STR " -> AVAILABLE", DRMDISPLAY_ID_PARAMS );
	    ...
	    Log::DTRACE( DRM_SUSPEND_DEBUG, DRMDISPLAY_ID_STR " DPMS_ON", DRMDISPLAY_ID_PARAMS );
            ...
            for (;;)
            {
                if ( !bOK )
                {
		    WTRACE( "DRM_MODE_DPMS_ASYNC_ON did not complete - forcing DRM_MODE_DPMS_ON" );
                    mDrm.setDPMSProperty( mDrmConnectorID, mPropDPMS, DRM_MODE_DPMS_ON );
                    break;
                }
                usleep( wait25Ms );
        ...
        if ( !mbScreenCtlOn )
        {
	    DTRACEIF( DRM_BLANKING_DEBUG, DRMDISPLAY_ID_STR " screen ctl = 1", DRMDISPLAY_ID_PARAMS );
...
void DrmDisplay::setBlanking( void )
{
    ...
    if ( !pBlankingFrame )
    {
	ETRACE( "Failed to create blanking frame" );
        return;
    }
    ...
    DTRACEIF( DRM_DISPLAY_DEBUG, "Flip custom frame %p (type eFT_BlankingFrame)", pBlankingFrame );
...
void DrmDisplay::releaseFlippedFrame( Frame* pOldFrame )
{
    ...
    HWCASSERT( pOldFrame->getType() == eFT_BlankingFrame );

    // Delete the frame.
    DTRACEIF( DRM_DISPLAY_DEBUG, "Delete custom frame %p (eFT_BlankingFrame)", pOldFrame );
    delete pOldFrame;
}
...
bool DrmDisplay::updateTiming( const DisplayQueue::Frame& frame )
{
    ...
    if (meStatus != SUSPENDED)
    {
        ...
        {
	    DTRACEIF( DRMDISPLAY_MODE_DEBUG, "Waiting to change to notified timing %u", timingIndex );
            Timing t;
...
int DrmDisplay::queueFrame( const Content::Display& display, uint32_t zorder, int* pRetireFenceFd )
{
    ...
    DTRACEIF( DRM_DISPLAY_DEBUG, DRMDISPLAY_ID_STR " Queue frame %u", DRMDISPLAY_ID_PARAMS, hwcFrameIndex );
    ...
    *pRetireFenceFd = mPageFlipHandler.registerNextFutureFrame( &timelineIndex );
    ...
    {
	ETRACE( "Failed DisplayQueue::queueFrame" );
