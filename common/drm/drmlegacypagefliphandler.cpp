...
{
    HWCASSERT( ( objectType != DRM_MODE_OBJECT_CRTC ) || ( objectID == crtcID ) );
    mDrmCrtcID = crtcID;
...
void DrmLegacyPageFlipHandler::Plane::flip( const Layer* pLayer, uint32_t flipEventData, bool* pbRequestedFlip )
{
    ...
    else
    {
	    ...
	    HWCASSERT( mDrmObjType != DRM_MODE_OBJECT_CRTC );
...
void DrmLegacyPageFlipHandler::doInit()
{
    ...
    if ( !maPlanes )
    {
	ETRACE( "Failed to create plane state" );
        return;
    }
    ...
    for ( uint32_t p = 0; p < mNumPlanes; ++p )
    {
        ...
        if ( bMain )
        {
	    HWCASSERT( mDisplay.getDrmCrtcID( ) == id );
	...
	DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u Plane %u : Id %u (%s)",
...
bool DrmLegacyPageFlipHandler::doFlip( DisplayQueue::Frame* pNewFrame, bool bMainBlanked, uint32_t flipEvData )
{
    ...
    if ( ((mFlips++) & 255) == 0 )
    {
	WTRACE( "WARNING: Non-atomic legacy drm in use, expect occasional flickers" );
	...
	ETRACE( "Failed to apply global scaling changes (panel fitter fail)" );
    }
