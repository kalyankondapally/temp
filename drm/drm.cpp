    ...
    mpModeRes(NULL)
{
    ...
    if (mbCapNuclear)
    {
	    ...
	    ITRACE( "Detected DMR/KMS Nuclear and Universal Plane support" );
...
#endif
    ITRACE( "%s DMR/KMS Render Compression support", mbCapRenderCompression ? "Detected" : "NOT AVAILABLE:" );
}
...
void Drm::init(GpuDevice& device)
{
    mDevice = &device;
    mpEventThread.reset(new DrmEventThread());
    if (mpEventThread == NULL)
    {
	ETRACE("Composer::Drm Failed to initialize DrmEventThread.");
        return;
    }

    mpUEventThread.reset(new DrmUEventThread(device, *this));
    if (mpUEventThread == NULL || !mpUEventThread->Initialize())
    {
	ETRACE("Composer::Drm Failed to initialize DrmUEventThread.");
    }
}
...
bool Drm::setActiveDisplay( uint32_t drmDisplay, bool bActive )
{
    ...
    DTRACEIF( DRM_PROBE_DEBUG, "Drm setActiveDisplay %u active %d", drmDisplay, bActive );

    ScopedSpinLock _l( mLockForCrtcMask );
    ...
    DTRACEIF( DRM_PROBE_DEBUG, "Drm setActiveDisplay 0x%x (x%u) sync:%d",
...
void Drm::onHotPlugEvent(UEvent eHPE)
{
    ...
    HWCASSERT( cMaxSupportedPhysicalDisplays <= 32 );
    uint32_t plug = 0;
    uint32_t unplug = 0;
    /*
...
void Drm::onESDEvent( UEvent eEvent, uint32_t connectorID, uint32_t connectorType)
{
    ...
    {
	ETRACE("Drm ESDEvent: skip since [%s] was passed into!", UEventToString(eEvent) );
        return;
    }
/*
    for ( uint32_t display = 0; display < cMaxSupportedPhysicalDisplays; ++display )
...
bool Drm::acquirePipe(uint32_t possible_crtcs, uint32_t& crtc_id, uint32_t& pipe_idx)
{
    ...
    DTRACEIF( DRM_PROBE_DEBUG, "Acquiring pipe from possible set 0x%x [Crtcs acquired mask 0x%x]", possible_crtcs, mAcquiredCrtcs );
    ...
    for (int i = 0; i < mpModeRes->count_crtcs; i++)
    {
        if ( ( possible_crtcs & (1 << i) ) && !( mAcquiredCrtcs & (1 << i) ) )
        {
            ...
            mAcquiredCrtcs |= (1 << i);
            ++mAcquiredPipes;
	    DTRACEIF( DRM_PROBE_DEBUG, "Acquired PipeIdx:%u CrtcID:%u. [Acquired Pipes %u, Crtcs Mask 0x%x]",
                pipe_idx, crtc_id, mAcquiredPipes, mAcquiredCrtcs );
            return true;
        }
    }
    WTRACE( "No pipes available [Crtcs acquired mask 0x%x]", mAcquiredCrtcs );
    return false;
}
...
void Drm::releasePipe(uint32_t pipe_idx)
{
    ScopedSpinLock _l( mLockForCrtcMask );
    HWCASSERT( mpModeRes );
    HWCASSERT( pipe_idx < (uint32_t)mpModeRes->count_crtcs );
    HWCASSERT( mAcquiredCrtcs & (1 << pipe_idx) );
    HWCASSERT( mAcquiredPipes > 0 );
    mAcquiredCrtcs &= ~(1 << pipe_idx);
    --mAcquiredPipes;
    DTRACEIF( DRM_PROBE_DEBUG, "Released PipeIdx:%u. [Acquired Pipes %u, Crtcs Mask 0x%x]",
        pipe_idx, mAcquiredPipes, mAcquiredCrtcs );
}

int Drm::probe(GpuDevice& device)
{
	...
	ETRACE("probe FAILED to get modeset resources");
        return BAD_VALUE;
    }
    ...
    for (uint32_t i = 0; i < (uint32_t)mpModeRes->count_connectors; i++)
    {
        ...
        if (pConnector == NULL)
        {
	    ITRACEIF( DRM_PROBE_DEBUG, "Invalid connector");
            continue;
        }
        ...
        {
	    ITRACEIF( DRM_PROBE_DEBUG, "DrmDisplay::probe() Skipping disabled internal connector type.");
        ...
        {
	    ITRACEIF( DRM_PROBE_DEBUG, "DrmDisplay::probe() Skipping disabled external connector type.");
            freeConnector( pConnector );
            continue;
        }
    ...
    // Plug connected.
    DTRACEIF( DRM_PROBE_DEBUG, "DrmDisplay::probe() New mapping:" );
   /* PhysicalDisplayManager& pdm = device.GetPhysicalDisplayManager( );
    for (uint32_t d = 0; d < cMaxSupportedPhysicalDisplays; d++)
    {
        if (mDisplay[d] != NULL)
        {
			...
			DTRACEIF ( DRM_PROBE_DEBUG,
                    ...
                    {
			DTRACEIF ( DRM_PROBE_DEBUG, "No available pipe found for display %u", d);
                    }
                }
            }
            // Summary log
...
{
    HWCASSERT(mpEventThread != NULL);
    HWCASSERT(pDisp != NULL);
...
void Drm::freeConnector(drmModeConnectorPtr ptr) {
  ...
  ETRACEIF(!ptr, "Missing connector ptr");
...
void Drm::freePlaneResources(drmModePlaneResPtr ptr) {
  ...
  ETRACEIF(!ptr, "Missing plane resources ptr");
...
void Drm::freePlane(drmModePlanePtr ptr) {
  ...
  ETRACEIF(!ptr, "Missing plane ptr");
...
#endif
  WTRACEIF(propId == INVALID_PROPERTY, "Panel fitter property not available");
  return propId;
}
...
uint32_t Drm::getPanelFitterSourceSizePropertyID(uint32_t connector_id) {
  ...
  HWC_UNUSED(connector_id);
#endif
  WTRACEIF(sbInternalBuild && propId == INVALID_PROPERTY,
...
uint32_t Drm::getPropertyID(uint32_t obj_id, uint32_t obj_type,
  ...
  DTRACEIF(DRM_STATE_DEBUG,
           "drmModeObjectGetProperties( obj_id %u, obj_type %u )", obj_id,
           obj_type);
  props = drmModeObjectGetProperties(mDrmFd, obj_id, obj_type);
  if (!props) {
    ETRACE("Display enumPropertyID - could not get connector properties");
    return -1;
  }

  for (uint32_t j = 0; j < props->count_props; j++) {
    drmModePropertyPtr prop;
    DTRACEIF(DRM_STATE_DEBUG, "drmModeGetProperty( property_id %u )",
             props->props[j]);
    prop = drmModeGetProperty(mDrmFd, props->props[j]);
    if (prop == NULL) {
      ETRACE("Get Property return NULL");
    ...
    DTRACEIF(DRM_STATE_DEBUG, "drmModeFreeProperty( ptr %p id:%d name:%s)",
  ...
  DTRACEIF(sbInternalBuild && prop_id == INVALID_PROPERTY,
...
int Drm::releasePanelFitter(uint32_t connector_id) {
  ...
  ETRACE("panel fitter not acquired for connector id %u", connector_id);
  return BAD_VALUE;
}
...
int Drm::setPanelFitterProperty(uint32_t connector_id, int32_t pfit_prop_id,
  ...
  if (pfit_prop_id == -1) {
    ETRACE("Panel fitter not available");
    return BAD_VALUE;
  }
  ...
  if (drmModeObjectSetProperty(mDrmFd, connector_id, DRM_MODE_OBJECT_CONNECTOR,
    ...
    ETRACE("set panel fitter property failed");
    return -1;
  }
  return 0;
  ...
  HWC_UNUSED(dstW);
  HWC_UNUSED(dstH);
  ETRACE("Panel fitter support missing");
  return BAD_VALUE;
#endif
}
...
int Drm::setPanelFitterSourceSizeProperty( uint32_t connector_id, int32_t pfit_prop_id, uint32_t srcW, uint32_t srcH )
{
    ...
    if ( pfit_prop_id == -1 )
    {
	ETRACEIF( sbInternalBuild, "Panel fitter source size not available" );
        return BAD_VALUE;
    }

    srcW--;
    srcH--;
    HWCASSERT( srcW<=0xFFFF );
    HWCASSERT( srcH<=0xFFFF );
    ...
    {
	ETRACE("set panel fitter source size property failed");
        return -1;
    }
    return 0;
    ...
    HWC_UNUSED( srcW );
    HWC_UNUSED( srcH );
    ETRACE( "Panel fitter source size support missing" );
    return BAD_VALUE;
#endif
}
...
int Drm::setDPMSProperty( uint32_t connector_id, int32_t prop_id, uint32_t mode )
{
    ...
    if ( res )
    {
       ETRACE("Set DPMS property failed");
       return -1;
    }
...
int Drm::setDecrypt( uint32_t objectType, uint32_t id, bool bEnable )
{
    ...
    {
	DTRACEIF(DRM_STATE_DEBUG, "setDecrypt for main display plane skipped (unsupported)" );
        return OK;
    }
...
int Drm::setZOrder( uint32_t crtc_id, uint32_t zorder )
{
    ...
    HWC_UNUSED(crtc_id);
    HWC_UNUSED(zorder);
    ETRACE("Plane ZOrder support missing");
    return ~SUCCESS;
...
int Drm::screenCtl(uint32_t crtc_id, uint32_t enable) {
  ...
  DTRACEIF(DRM_STATE_DEBUG && ret != SUCCESS,
...
int Drm::setTransform( uint32_t objectType, uint32_t id, ETransform transform )
{
    ...
    HWCASSERT( ( objectType == DRM_MODE_OBJECT_CRTC )
              || ( objectType == DRM_MODE_OBJECT_PLANE ) );

#if VPG_DRM_HAVE_TRANSFORM_180
    HWCASSERT( ( transform == ETransform::NONE ) || ( transform == ETransform::ROT_180 ) );
...
bool Drm::isSupportedInternalConnectorType( uint32_t connectorType ) const
{
    ...
    DTRACEIF( DRM_STATE_DEBUG, "Drm::isSupportedInternalConnectorType, connectorType=%d, support=%s",connectorType, ( ( isSupported == true )? "Yes" : "No" ) );
    return isSupported;
}
...
bool Drm::isSupportedExternalConnectorType( uint32_t connectorType ) const
{
    ...
    DTRACEIF( DRM_STATE_DEBUG, "Drm::isSupportedExternalConnectorType, connectorType=%d, support=%s",connectorType, ( ( isSupported == true )? "Yes" : "No" ) );
    return isSupported;
...
Drm::Blob* Drm::Blob::create( Drm& drm, const void* pData, uint32_t size )
{
    Blob* ret = NULL;
#ifdef DRM_IOCTL_MODE_CREATEPROPBLOB
    uint32_t blob_id = 0;
    drmModeCreatePropertyBlob(drm.getDrmHandle(), &pData, size,
			      &blob_id);
    if (blob_id != 0)
    {
	ret = new Blob(drm, blob_id);
    }
#else
...
Drm::Blob::~Blob()
{
    ...
    if (mID) {
      drmModeDestroyPropertyBlob(mDrm.getDrmHandle(), mID);
      mID = 0;
    }
#else
    HWC_UNUSED(mDrm);
...
Drm::Blob* Drm::createBlob( const void* pData, uint32_t size )
{
