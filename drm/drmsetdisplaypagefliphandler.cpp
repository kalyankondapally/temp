...
bool DrmSetDisplayPageFlipHandler::test( DrmDisplay& display )
{
    ...
    {
	ITRACE( "Drm atomic API is disabled" );
        return false;
    }
    else if ( sOptionSetDisplay.get() == eSetDisplayEnabled )
    {
	ITRACE( "Drm atomic API is enabled" );
        return true;
    }
    ...
    DTRACE( "Testing Drm atomic API" );

    int err = Drm::get().drmSetDisplay( setDisplay );
    if ( err == Drm::SUCCESS )
    {
	DTRACE( "Drm atomic API is available" );
    ...
    DTRACE( "Drm atomic API is not available" );
...
bool DrmSetDisplayPageFlipHandler::doFlip( DisplayQueue::Frame* pNewFrame, bool bMainBlanked, uint32_t flipEvData )
{
	...
	DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " updatePlane %u flipEventData 0x%x, bRequestedFlip %d",
    ...
    ETRACEIF( !bRequestedFlip, "Failed to issue flip event request for frame %s", pNewFrame->getFrameId().dump().string() );
	...
	HWCASSERT( !mSetDisplay.plane[ mMainPlaneIndex ].rrb2_enable );
        mSetDisplay.plane[ mMainPlaneIndex ].update_flag &= ~DRM_MODE_SET_DISPLAY_PLANE_UPDATE_RRB2;
    }

    DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " %s", Drm::drmDisplayPipeToString( mSetDisplay ).string( ) );
    for ( uint32_t p = 0; p < mSetDisplay.num_planes; ++p )
    {
	DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME "     %s", Drm::drmDisplayPlaneToString( mSetDisplay, p ).string( ) );
    }
...
{
    HWCASSERT( pPlane );
    DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u Update %s %u",
	...
	HWCASSERT( ( hwcTransform == ETransform::NONE ) || ( hwcTransform == ETransform::ROT_180 ) );
	    ...
	    DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u  fb -> %d src:%.2f,%.2f %.2fx%.2f -> dst:%d,%d %dx%d ud:0x%llx",
            ...
            if ( pPlane->alpha != bAlpha )
            {
                ...
                pPlane->update_flag |= DRM_MODE_SET_DISPLAY_PLANE_UPDATE_ALPHA;
                pPlane->alpha = bAlpha;
		DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u  alpha -> %d", mDisplay.getDrmCrtcID(), bAlpha );
            }
            ...
            if ( pPlane->rrb2_enable != bRrb2 )
            {
                ...
                pPlane->rrb2_enable = bRrb2;
		DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u  rrb2 -> %d", mDisplay.getDrmCrtcID(), bRrb2 );
            }
            ...
            if ( pPlane->transform != drmTransform )
            {
                ...
                pPlane->transform = drmTransform;
		DTRACEIF( DRM_PAGEFLIP_DEBUG, DRM_PFH_NAME " Crtc %u  transform -> %d", mDisplay.getDrmCrtcID(), drmTransform );
            }
...
void DrmSetDisplayPageFlipHandler::validateSetDisplay( void )
{
    ...
    for( uint32_t p = 0; p < mNumPlanes; ++p )
    {
        if ( mSetDisplay.update_flag & DRM_MODE_SET_DISPLAY_UPDATE_PLANE( p ) )
        {
            ...
            {
		ETRACE( "Plane %u has plane update flag set but no plane-specific dirty bits set", p );
		HWCASSERT( false );
            }
            ...
            if ( mSetDisplay.plane[p].flags & DRM_MODE_PAGE_FLIP_EVENT )
            {
                ...
                {
		    ETRACE( "Plane %u has DRM_MODE_PAGE_FLIP_EVENT set but user data is not set", p );
		    HWCASSERT( false );
                }
                if ( !(mSetDisplay.plane[p].update_flag & DRM_MODE_SET_DISPLAY_PLANE_UPDATE_PRESENT ) )
                {
		    ETRACE( "Plane %u has DRM_MODE_PAGE_FLIP_EVENT set but presentation flag is not set", p );
		    HWCASSERT( false );
                }
                bHaveFlip = true;
            }
        ...
        else
        {
            ...
            if ( mSetDisplay.plane[p].update_flag & DRM_MODE_SET_DISPLAY_PLANE_UPDATE_PRESENT )
            {
		ETRACE( "Plane %u is not flagged for update but has plane-specific update flag set", p );
		HWCASSERT( false );
            }
            ...
            {
		ETRACE( "Plane %u is enabled but is not flagged for update and has plane-specific dirty bits set", p );
		HWCASSERT( false );
            }
            if ( mSetDisplay.plane[p].flags & DRM_MODE_PAGE_FLIP_EVENT )
            {
		ETRACE( "Plane %u is not flagged for update but has DRM_MODE_PAGE_FLIP_EVENT set", p );
		HWCASSERT( false );
            }
            if ( mSetDisplay.plane[p].user_data != 0ULL )
            {
		ETRACE( "Plane %u is not flagged for update but has user_data set", p );
		HWCASSERT( false );
            }
        }
    }
	...
	ETRACE( "Did not set DRM_MODE_PAGE_FLIP_EVENT for any active presented plane" );
    }
}
#endif
